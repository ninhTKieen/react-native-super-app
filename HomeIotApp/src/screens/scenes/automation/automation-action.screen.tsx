...
const AutomationActionScreen = () => {
  const navigation =
    useNavigation<StackNavigationProp<CreateAutomationStackParamList>>();
  const uNavigation =
    useNavigation<StackNavigationProp<UpdateAutomationStackParamList>>();
  const route =
    useRoute<
      RouteProp<CreateAutomationStackParamList, 'CreateAutomationAction'>
    >();
  ...
  const millisecondsToHMS = useCallback((ms: number) => {
        ...
        {type === 'RUN_DEVICE' && (
          <ActionDeviceList
            onPressDeviceItem={(device) => {
              if (mode === 'CREATE') {
                navigation.replace('CreateSelectDeviceActionFunction', {
                  device,
                });
              } else if (mode === 'UPDATE') {
                uNavigation.replace('UpdateSelectDeviceActionFunction', {
                  device,
                });
              }
            }}
...
export default AutomationActionScreen;
