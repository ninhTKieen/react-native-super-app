...
const VoucherItemIncoming = ({item}: Props) => {
  ...
  const {mutate: deleteVoucher} = useMutation({
    mutationFn: () => voucherService.deleteVoucher({id: item.id}),
    onSuccess: () => {
      ...
      Toast.show({
        ...
        text2: t(i18nKeys.voucher.deleteSuccess) as string,
      });
    },
    onError: () => {
      Toast.show({
        ...
        text2: t(i18nKeys.voucher.deleteFail) as string,
      });
    },
    onSettled: () => {
  ...
  const {mutate: startVoucher} = useMutation({
    mutationFn: () => voucherService.startVoucher({id: item.id}),
    onSuccess: () => {
      queryClient.refetchQueries(['vouchers', '22']);
      ...
      Toast.show({
        type: 'success',
        text1: t(i18nKeys.common.success) as string,
        text2: t(i18nKeys.voucher.startSuccess) as string,
      });
    },
    onError: () => {
      Toast.show({
        type: 'error',
        text1: t(i18nKeys.common.error) as string,
        text2: t(i18nKeys.voucher.startFail) as string,
      });
    },
    onSettled: () => {
      closeModal('LoadingModal');
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.action}>
        <View style={styles.button_wrapper}>
          <Button
            ...
            onPress={() => startVoucher()}>
            {t(i18nKeys.common.start)}
          </Button>
        </View>
        <View style={styles.button_wrapper}>
          <Button
      ...
      <ConfirmModal
        ...
        message={t(i18nKeys.voucher.confirmDelete)}
...
export default VoucherItemIncoming;
